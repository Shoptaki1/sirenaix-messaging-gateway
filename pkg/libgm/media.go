...
func (c *Client) FinalizeUploadMedia(upload *StartGoogleUpload) (*MediaUpload, error) {
	...
	respData, err := readProviderResponseBounded(req.Context(), res.Body, maxUploadResponseBytes)
	if err != nil {
...
func (c *Client) DownloadMedia(mediaID string, key []byte) ([]byte, error) {
	...
	defer res.Body.Close()
	respData, err := readProviderResponseBounded(req.Context(), res.Body, maxGatewayMediaBytes+32)
	if err != nil {
		return nil, fmt.Errorf("failed to read response: %w", err)
	}
...
func (c *Client) DownloadAvatar(ctx context.Context, url string) ([]byte, error) {
	...
	return readProviderResponseBounded(ctx, resp.Body, maxGatewayMediaBytes)
}
